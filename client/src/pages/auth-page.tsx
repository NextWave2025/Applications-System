      ...
      <main className="flex-grow flex flex-col items-center justify-center px-4 py-16">
        <div className="w-full max-w-md">
          {window.location.pathname === '/login' ? (
            <LoginForm onLogin={(credentials) => loginMutation.mutate(credentials)} />
          ) : (
            <SignupForm onRegister={(userData) => registerMutation.mutate(userData)} />
          )}
        </div>
      </main>
    </div>
...
function LoginForm({ onLogin }: { onLogin: (credentials: { username: string; password: string }) => void }) {
...
function SignupForm({ onRegister }: { onRegister: (userData: any) => void }) {
